...
/// 通知画面情報クラスです。
/// </summary>
internal sealed class MessageScreenData : AbstractScreenData, OperateScreenData {
	#region メンバー変数定義
	/// <summary>
	/// 表題内容
	/// </summary>
	...
	public MessageScreenData() {
		this.headerText = "情報確認";
		this.detailText = "メッセージが○○様より届いております。\r\nグループウェアより確認してください。";
	...
	private void ActionInvokeMenu() {
		if (String.IsNullOrEmpty(this.headerText)) {
			...
			RemarkText = null;
			this.listenList?.Invoke(this, new MessageDialogData(this.headerText, this.detailText));
		}
	}
	#endregion 内部メソッド定義
}
